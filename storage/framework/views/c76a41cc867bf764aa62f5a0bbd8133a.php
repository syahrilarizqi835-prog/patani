<?php $__env->startSection('page-title', 'Chatbot AI'); ?>
...
<div class="space-y-4" x-data="{
    ...
    input: '',
    loading: false,
    isPremium: <?php echo e($isPremium ? 'true' : 'false'); ?>,
    sisaPesan: <?php echo e($sisaPesan ?? 'null'); ?>,
    ...
    async kirim() {
        ...
        try {
            const resp = await fetch('<?php echo e(route('dashboard.chatbot.send')); ?>', {
                method: 'POST',
                headers: {
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2 bg-yellow-50 border border-yellow-300 text-yellow-800 px-4 py-2 rounded-full font-semibold text-sm">
            ...
            <span class="text-xs font-normal">s/d <?php echo e(\Carbon\Carbon::parse(Auth::user()->premium_until)->format('d M Y')); ?></span>
            <?php endif; ?>
        </div>
        <?php else: ?>
        <div class="flex items-center gap-2 bg-gray-100 border border-gray-300 text-gray-600 px-4 py-2 rounded-full text-sm">
            ...
            <span class="font-bold" :class="sisaPesan <= 1 ? 'text-red-600' : 'text-green-600'"
                x-text="sisaPesan + '/<?php echo e(\App\Http\Controllers\Petani\ChatbotController::BATAS_GRATIS); ?>'"></span>
        </div>
        <?php endif; ?>
    </div>
        ...
        <div id="chatbox" class="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div class="flex gap-3">
                ...
                <div class="bg-gray-100 rounded-2xl rounded-tl-none px-4 py-3 max-w-md">
                    <p class="text-gray-800 text-sm">Halo <strong><?php echo e(Auth::user()->name); ?></strong>! Saya PATANI Assistant siap membantu konsultasi pertanian Anda.</p>
                    <?php if($sawahList->count() > 0): ?>
                    <p class="text-gray-600 text-xs mt-2">Saya sudah mengetahui data sawah Anda: <strong><?php echo e($sawahList->pluck('nama_sawah')->join(', ')); ?></strong>. Tanyakan apa saja!</p>
                    <?php endif; ?>
                </div>
            </div>

            
            <?php $__currentLoopData = $riwayat; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $chat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="flex gap-3">
                ...
                <div class="bg-gray-100 rounded-2xl rounded-tl-none px-4 py-3 max-w-lg">
                    <p class="text-gray-800 text-sm whitespace-pre-line"><?php echo e($chat->response); ?></p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow border border-gray-200 p-4">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            ?>
            <?php $__currentLoopData = $quickQ; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $q): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button @click="tanyaCepat('<?php echo e($q); ?>')"
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\patani\laravel-patani\resources\views/dashboard/chatbot.blade.php ENDPATH**/ ?>